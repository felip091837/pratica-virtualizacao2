...
cp cirros.img red2.img
cp cirros.img green2.img

#cria interfaces TAP
ip tuntap add tap-red2 mode tap user ubuntu
ip tuntap add tap-green2 mode tap user ubuntu

#instancia as duas VMs
qemu-system-x86_64 -device e1000,netdev=user0 -netdev user,id=user0,hostfwd=tcp::2221-:22 -device e1000,netdev=net0,mac=00:00:00:00:00:02 -netdev tap,id=net0,ifname=tap-red2,script=no,downscript=no -m 256 -drive file=red2.img,media=disk,cache=writeback -vnc :1 -daemonize
qemu-system-x86_64 -device e1000,netdev=user0 -netdev user,id=user0,hostfwd=tcp::2222-:22 -device e1000,netdev=net0,mac=00:00:00:00:00:02 -netdev tap,id=net0,ifname=tap-green2,script=no,downscript=no -m 256 -drive file=green2.img,media=disk,cache=writeback -vnc :2 -daemonize
...
ip link set tap-red2 up
ip link set tap-green2 up
...
ovs-vsctl add-port br-red tap-red2
ovs-vsctl add-port br-green tap-green2

#adiciona interface VTEP a br-red e a br-green
ovs-vsctl add-port br-red vxlan0 -- set interface vxlan0 type=vxlan options:remote_ip=$IP_VM1 options:key=9001 options:dst_port=9001
ovs-vsctl add-port br-green vxlan1 -- set interface vxlan1 type=vxlan options:remote_ip=$IP_VM1 options:key=9002 options:dst_port=9002
